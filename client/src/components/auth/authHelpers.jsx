...
const saveAdminToLocal = (admin) => {
  if (admin) {
    localStorage.setItem("provinceAdmin", JSON.stringify(admin)); // Store as a simple string
  } else {
    localStorage.removeItem("provinceAdmin");
  }
};
...
const getAdminFromLocal = () => {
  return JSON.parse(localStorage.getItem("provinceAdmin")) || null;
};
...
function openDatabase() {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains("provinceAdminTokens")) {
        db.createObjectStore("provinceAdminTokens", { keyPath: "id" });
      }
    };
...
async function storeRefreshToken(token) {
  ...
  const transaction = db.transaction("provinceAdminTokens", "readwrite");
  const store = transaction.objectStore("provinceAdminTokens");
...
async function getRefreshTokenFromDb() {
  const db = await openDatabase();
  const transaction = db.transaction("provinceAdminTokens", "readonly");
  const store = transaction.objectStore("provinceAdminTokens");
...
async function deleteRefreshTokenFromDb() {
  const db = await openDatabase();
  const transaction = db.transaction("provinceAdminTokens", "readwrite"); // Use readwrite for deletion
  const store = transaction.objectStore("provinceAdminTokens");
